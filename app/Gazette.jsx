...
태양광 발전량을 더 정확히 예측하면 배터리(ESS)를 더 잘 굴려 돈을 더 벌 것이다 — 당연해 보이는 가정에서 출발했습니다. 평소 환경을 위한 기술에 관심이 많아 태양광 발전량을 프로젝트 주제로 골랐고, 태양광 에너지를 어떻게 하면 실제 수익으로 만들어낼 수 있을지 전국 17개 시도의 1년치 데이터로 직접 풀어보고 싶었습니다.
...
그래서 모델은 그대로 두고 운영 방식만 바꿨습니다. '남으면 충전, 모자라면 방전'이라는 단순한 규칙을, 24시간을 내다보고 충방전 시점을 통째로 계산하는 MPC로 교체했습니다. 그러자 같은 데이터, 같은 예측인데 순수익이 **+49.5%** 올랐습니다(1,689억 → 2,526억원). 완벽한 예측을 썼을 때의 수익 개선 효과는 0.08%에 그쳤지만, 운영 구조를 바꾼 효과는 49.5%였습니다. 결과를 가른 건 모델이 아니라 시스템 구조였습니다.
